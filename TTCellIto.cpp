...
double TTCellIto<ncells>::comp_ical (int id, double dt, double& dd, double& df, double& dfca) // L-type Calcium Current
{
    ...
    if (fabs(vfrt) < 1e-3) {
        ical = icalfac[id]*gcal*d[id]*f[id]*fcass*zca*frdy*(cai[id]*exp(vfrt) - 0.341*cao)/(1.0 + vfrt/2.0 + vfrt*vfrt/6.0 + vfrt*vfrt*vfrt/24.0);
    }
    else {
        ical = icalfac[id]*gcal*d[id]*f[id]*fcass*zca*frdy*vfrt*(cai[id]*exp(vfrt) - 0.341*cao)/(exp(vfrt) - 1.0);
    }
#else    
    ...
    if (fabs(vfrt) < 1e-3) {
        ical = icalfac[id]*gcal*d[id]*f[id]*fca[id]*zca*frdy*(cai[id]*exp(vfrt) - 0.341*cao)/(1.0 + vfrt/2.0 + vfrt*vfrt/6.0 + vfrt*vfrt*vfrt/24.0);
    }
    else {
        ical = icalfac[id]*gcal*d[id]*f[id]*fca[id]*zca*frdy*vfrt*(cai[id]*exp(vfrt) - 0.341*cao)/(exp(vfrt) - 1.0);
    }
